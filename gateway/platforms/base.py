...
class SendResult:
    ...
    # :func:`classify_send_error`. ``"edit_message_not_found"`` is the
    # narrower edit-target discriminator: adapters set it only when a stored
    # message id is gone, so durable projections can recreate that message
    # once without treating a missing chat/thread as recoverable.
...
class BasePlatformAdapter(ABC):
    ...
    # Injected by GatewayRunner after adapter creation. build_source uses it to
    # resolve per-profile routing for every platform adapter.
    ...
    async def _send_with_retry(
        ...
        # A provider-issued flood deadline is authoritative.  Retrying a
        # differently formatted version below would still be the same Bot API
        # write, so it only extends the cooldown and turns one response into a
        # burst.  Some adapter paths only preserve the provider's textual 429
        # error, so recognise both the explicit receipt and its canonical
        # classification instead of falling through to a plain-text resend.
        # The caller can defer the delivery using ``retry_after`` when present.
        if (
            result.retry_after is not None and result.retry_after > 5.0
        ) or result.error_kind == "rate_limited" or classify_send_error(error_str) == "rate_limited":
            return result
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # Bare paths are prose, not an attachment command. Requiring
                # an explicit MEDIA: directive prevents internal manifests,
                # JSON, and Markdown working files from being sent to chat.
                local_files = []
    ...
    def build_source(
        ...
        """Build a SessionSource and stamp the configured profile route."""
