...
def test_sigterm_with_kanban_task_env_terminates_quickly():
    ...
    try:
        ...
        deadline = t0 + 2.0
        while time.time() < deadline:
            # On macOS a terminated child stays visible to ``kill(pid, 0)``
            # as a zombie until its Popen parent reaps it. Poll first so the
            # test measures the worker's exit, not that platform detail.
