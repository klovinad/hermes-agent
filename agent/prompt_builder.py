...
KANBAN_GUIDANCE = (
    ...
    "A successful `kanban_complete` or review handoff is your final state-changing "
    "operation: do not launch tests, shell commands, background jobs, or further "
    "tool calls after it. Return your final response and exit so the dispatcher can "
    "reliably stop this worker before any reviewer or rework worker starts. "
    "For a UI bug or automation task, preserve the approved visible UI and its "
    "existing interaction contract. Do not add a new button, icon, drag handle, "
    "label, or other visible affordance merely to make implementation or testing "
    "easier. First reproduce the existing intended interaction; if it cannot be "
    "made reliable without a product-design change, block with evidence and ask the "
    "orchestrator for that decision. Tests must exercise the agreed user behavior, "
    "not a new UI invented by the test implementation. "
    ...
    "- **Visual QA preflight.** Before creating a Light evidence child, inspect the "
    "target implementation to identify the actual gesture and check every installed "
    "suitable runner, not just the first harness tried. A limitation of Maestro, "
    "Playwright, or another individual runner is NOT an automation block when a "
    "native runner (for example XCUITest) can perform the interaction. Record the "
    "failed runner, the viable alternative, and route the evidence task to Light. "
    "Only use `kanban_block` with an automation reason after source inspection and "
    "all suitable installed runners have been ruled out with concrete evidence. If "
    "a capability fix is genuinely needed, create it as a parent; flow preparation "
    "and Light evidence must depend on it.\n"
    "\n"
    "## Do NOT\n"
    "\n"
